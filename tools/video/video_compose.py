...
- `motion_canvas` → diagram/map/code/math/timeline inserts via
                 `motion_canvas_compose`. Job lock usually stays remotion
                 with `compose_strategy: scene_assemble`.
...
class VideoCompose(BaseTool):
    ...
    def get_info(self) -> dict[str, Any]:
        ...
        try:
            ...
            from tools.video.motion_canvas_compose import MotionCanvasCompose

            mc_ok = MotionCanvasCompose().get_status() == ToolStatus.AVAILABLE
        except Exception:
            mc_ok = False
        info["render_engines"] = {
            "ffmpeg": ffmpeg_ok,
    ...
    def _stage_remotion_media(value: Any, public_dir: Path) -> int:
        ...
        media_keys = {"source", "src", "backgroundSrc", "videoSrc", "audioSrc"}
    ...
    def _render_via_atelier(
        ...
        try:
            ...
            progress.note(f"npx remotion render {comp_id} -> {output_path.name} (up to 30 min)", kind="render")
    ...
    def _render(self, inputs: dict[str, Any]) -> ToolResult:
        ...
        if not render_runtime:
            return ToolResult(
                ...
                error=(
                    ...
                    "'hyperframes', 'ffmpeg', 'motion_canvas'. Re-run the proposal "
                    "stage with an explicit runtime choice — do NOT default this field."
                ),
            )

        if render_runtime not in {"remotion", "hyperframes", "ffmpeg", "motion_canvas"}:
            return ToolResult(
                success=False,
                error=(
                    f"Unknown render_runtime {render_runtime!r}. "
                    f"Valid values: remotion, hyperframes, ffmpeg, motion_canvas. "
        ...
        if render_runtime == "motion_canvas":
            return self._render_via_motion_canvas(
                inputs=inputs,
                edit_decisions=edit_decisions,
                output_path=output_path,
            )
    ...
    def _render_via_motion_canvas(
        ...
        *,
        ...
        """Route a locked motion_canvas render to the adapter tool."""
        try:
            from tools.video.motion_canvas_compose import MotionCanvasCompose
        except Exception as exc:
            return ToolResult(
                success=False,
                error=f"Could not import motion_canvas_compose: {exc}",
            )
        workspace = Path(
            inputs.get("workspace_path")
            or str(output_path.parent.parent / "motion_canvas")
        )
        scene = {}
        for row in edit_decisions.get("cuts") or []:
            if isinstance(row, dict):
                scene = row
                break
        result = MotionCanvasCompose().execute(
            {
                "operation": "render",
                "workspace_path": str(workspace),
                "output_path": str(output_path),
                "scene_type": str(
                    inputs.get("scene_type")
                    or scene.get("type")
                    or (edit_decisions.get("metadata") or {}).get("scene_type")
                    or "diagram"
                ),
                "title": str(
                    inputs.get("title")
                    or (edit_decisions.get("metadata") or {}).get("title")
                    or "Scene"
                ),
                "duration_seconds": float(inputs.get("duration_seconds") or 5),
                "timestamps": inputs.get("timestamps") or [],
            }
        )
        if not result.success:
            return ToolResult(
                success=False,
                error=result.error or "motion_canvas_compose failed",
    ...
    def _remotion_render(self, inputs: dict[str, Any]) -> ToolResult:
        ...
        # Honor an explicit composition_id from the SaaS runner (HeroTitle,
        # TerminalScene, EndTag, TalkingHead). Otherwise map renderer_family.
        explicit_id = inputs.get("composition_id")
        if explicit_id:
            composition_id = str(explicit_id)
        else:
        ...
        try:
            ...
            duration_frames = inputs.get("duration_frames")
            if duration_frames:
                try:
                    n = max(1, int(duration_frames))
                    cmd.append(f"--frames=0-{n - 1}")
                except (TypeError, ValueError):
                    pass
            if inputs.get("width"):
                cmd.extend(["--width", str(int(inputs["width"]))])
            if inputs.get("height"):
                cmd.extend(["--height", str(int(inputs["height"]))])
            ...
            progress.note(
                f"npx remotion render {composition_id} -> {output_path.name} timeout={subprocess_timeout}s",
                kind="render",
            )
