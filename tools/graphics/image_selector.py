...
class ImageSelector(BaseTool):
    ...
    def _filter_candidates(self, inputs: dict[str, Any], candidates: list[BaseTool]) -> list[BaseTool]:
        ...
        candidates = [
            tool
            for tool in candidates
            if tool.name not in blocked and getattr(tool, "runtime", None) != ToolRuntime.LOCAL_GPU
        ]
    ...
    def _tool_selectable(self, tool: BaseTool, inputs: dict[str, Any]) -> bool:
        ...
        if tool.name == "local_diffusion" or getattr(tool, "runtime", None) == ToolRuntime.LOCAL_GPU:
            return False
