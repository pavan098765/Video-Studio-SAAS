...
class GoogleImagen(BaseTool):
    ...
    def execute(self, inputs: dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            # AI Studio :predict 404s on Imagen 4; Gemini native image still works on API keys.
            if (
                api_key
                and not str(model).startswith("gemini-")
                and ("404" in str(e) or "Not Found" in str(e))
            ):
                fallback = dict(inputs)
                fallback["model"] = "gemini-2.5-flash-image"
                return self._execute_gemini(fallback, "gemini-2.5-flash-image")
