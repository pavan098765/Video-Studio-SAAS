...
Run this AT THE ASSETS GATE (before any draft/compose render):
    ...
    python scripts/atelier_snapshots.py --project-dir /path/to/job/project --composition-id Scene
...
SaaS jobs pass --project-dir so stills land in the job workspace, not repo/projects/<slug>.
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("slug", nargs="?", default="", help="project slug under projects/")
    ap.add_argument("--project-dir", help="absolute project directory (SaaS job workspace)")
    ...
    if args.project_dir:
        ...
        print("error: pass a slug or --project-dir", file=sys.stderr)
        return 2
    if not proj.is_dir():
    ...
    wanted = [sc for sc in scenes if str(sc.get("id") or "").strip() and (not args.only or str(sc.get("id")) in args.only)]
    ok, fail = 0, 0
    for i, sc in enumerate(wanted, start=1):
        ...
        print(
            f"  snapshot {i}/{len(wanted)} {sid}: remotion still frame {frame}"
            + (" (webpack on first still)" if i == 1 else ""),
            flush=True,
        )
        cmd = [
        ...
        try:
            ...
            ok += 1
            print(f"  {sid}: frame {frame} -> {_rel(out)}")
    ...
    print(f"snapshots: {ok} ok, {fail} failed -> {_rel(snap_dir)}")
