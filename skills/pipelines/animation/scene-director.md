...
If the scene type is `diagram`, `map`, `timeline`, or `etymology`, include a real `mermaid` definition (not a 3-node stub). Code scenes need `code_snippet`. Math scenes need `formula_tex`.
