...
If the scene type is `diagram`, `map`, `timeline`, or `etymology`, you MUST include a real `mermaid` definition (not a 3-node A→B→C stub). Code scenes need `code_snippet`. Math scenes need `formula_tex`.
