...
1. **Which composition runtime should this video use?** — Remotion, HyperFrames, Motion Canvas, or FFmpeg.
...
Apply when:
- **Proposal stage** needs to lock `render_runtime` (remotion / hyperframes / ffmpeg / motion_canvas)
...
contract violation. Motion Canvas is an **insert runtime** for diagram/map/code/math
scenes on documentary, explainer, and hybrid jobs. Do not use it for TalkingHead,
karaoke-only captions, HyperFrames character rigs, or FFmpeg concat. Mixed
engines are allowed only when `compose_strategy: scene_assemble` is logged with
`scene_runtimes[]`.
...
|---|---|---|
| Existing React scene stack (text_card, stat_card, chart, caption overlay, TalkingHead, CinematicRenderer, TerminalScene) | **remotion** | `skills/core/remotion.md` |
...
| Diagram, map, etymology, timeline, code (Lezer), or math (TeX) inserts synced to narration | **motion_canvas** (per-scene; job lock stays remotion + `compose_strategy: scene_assemble`) | `tools/video/motion_canvas_compose.py` |
