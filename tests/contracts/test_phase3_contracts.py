...
class TestRemotionScaffold:
    ...
    def test_tailwind_is_enabled_for_atelier_and_stock(self):
        pkg = (self.REMOTION_DIR / "package.json").read_text(encoding="utf-8")
        assert "@remotion/tailwind-v4" in pkg
        assert "tailwindcss" in pkg
        cfg = (self.REMOTION_DIR / "remotion.config.ts").read_text(encoding="utf-8")
        assert "enableTailwind" in cfg
        assert "overrideWebpackConfig" in cfg
        css = (self.REMOTION_DIR / "src" / "index.css").read_text(encoding="utf-8")
        assert "tailwindcss" in css
        root = (self.REMOTION_DIR / "src" / "Root.tsx").read_text(encoding="utf-8")
        assert "index.css" in root
